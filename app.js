...
function saveDesk() { localStorage.setItem("buon_desk", JSON.stringify({ sizeUsd: $("sizeUsd").value, keepUsd: $("keepUsd").value, minAlert: $("minAlert").value, minOverlap: $("minOverlap").value, autoBuy: $("autoBuy").checked })); localStorage.setItem("buon_sizeUsd", $("sizeUsd").value); localStorage.setItem("buon_keepUsd", $("keepUsd").value); localStorage.setItem("buon_minAlert", $("minAlert").value); localStorage.setItem("buon_minOverlap", $("minOverlap").value); }
function loadDesk() { try { const raw = JSON.parse(localStorage.getItem("buon_desk") || "{}"); if (raw.sizeUsd) $("sizeUsd").value = raw.sizeUsd; if (raw.keepUsd != null && $("keepUsd")) $("keepUsd").value = raw.keepUsd; if (raw.minAlert) $("minAlert").value = raw.minAlert; if (raw.minOverlap) $("minOverlap").value = raw.minOverlap; if (raw.autoBuy) $("autoBuy").checked = raw.autoBuy; } catch (_) {} if ($("tapeKey")) $("tapeKey").value = state.key; }
...
["sizeUsd", "keepUsd", "minAlert", "minOverlap"].forEach((id) => { if ($(id)) $(id).onchange = saveDesk; });
